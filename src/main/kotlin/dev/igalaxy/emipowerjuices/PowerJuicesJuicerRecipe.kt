...
class PowerJuicesJuicerRecipe(private val input: List<EmiIngredient>, private val output: EmiStack, private val id: Identifier) : EmiRecipe {
    ...
    override fun getInputs(): MutableList<EmiIngredient> {
        ...
        while (newInputs.size < 4) {
            newInputs.add(EmiStack.EMPTY)
        }
    ...
    override fun addWidgets(widgets: WidgetHolder) {
        ...
        widgets.addSlot(inputs.getOrElse(0) { EmiIngredient.of(Ingredient.EMPTY) }, 1, 4).drawBack(false)
        widgets.addSlot(inputs.getOrElse(1) { EmiIngredient.of(Ingredient.EMPTY) }, 19, 4).drawBack(false)
        widgets.addSlot(inputs.getOrElse(2) { EmiIngredient.of(Ingredient.EMPTY) }, 37, 4).drawBack(false)
        widgets.addSlot(inputs.getOrElse(3) { EmiIngredient.of(Ingredient.EMPTY) }, 55, 4).drawBack(false)
        widgets.addSlot(inputs.getOrElse(4) { EmiIngredient.of(Ingredient.EMPTY) }, 73, 4).drawBack(false)
